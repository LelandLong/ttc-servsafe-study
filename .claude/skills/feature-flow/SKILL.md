...
>   - ⏱️ **STANDING WINDOW (Leland, 2026-08-11 — EXPIRES when fall term starts, ~Aug 21 2026):** while no classes are in session, **ALL changes ship live immediately, always** — full chain (docs + version bump + commit → PR → merge → verify live) runs straight through with no per-change ask. When students are back on the app this lapses and the ask-first default above returns. A conditional rule carries its own expiry: **if term has started and this note is still here, the note is stale and the gate wins.**

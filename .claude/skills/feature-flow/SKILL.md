...
> Crossfeed flow adaptation: `feature-flow` template v1.3.2 (`~/crossfeed/flows/`). This copy
...
>   - 🚀 **A FIX HE ASKED FOR IS ALREADY APPROVED** (Leland, 2026-09-12). A bug he reported or a fix he directed ("you are supposed to update that value so we can know precisely which version we are running") is the go: verify it fully, then run the whole chain to live. Holding verified PR #68 for a merge go drew *"don't know why you held off, should have been live already!"* Ask-first still applies to work he did NOT request: new features/UX, destructive prod-data changes, outward-facing content of a new kind he has not seen.
>   - ⏱️ **~~STANDING WINDOW~~ — EXPIRED 2026-08-21, ask-before-commit is IN FORCE.** Leland granted a ship-immediately window on 2026-08-11 that lapsed when fall term started (first HOS-190 class Fri 21 Aug). Kept, struck through, rather than deleted: the note carried its own expiry (*"if term has started and this note is still here, the note is stale and the gate wins"*) and that clause did its job — but a self-invalidating rule still has to be invalidated by someone, or every future session re-reads a live-looking grant. **Found 2026-08-27 by applying template v1.3.2 to this very file, six days after it expired.**
...
>   - 📡 **…and that extends to CROSSFEED: pointers, not payloads.** Report private-page work by slug and by what changed structurally ("hotels + phones updated on all three pages"), never by pasting the values. On 2026-08-19 this seat put the corrected Italy hotel names, addresses and phone numbers inline in CHEF-030 (TTC-025 did the same); XFD shipped a cloud mirror of the channel *the same day*, and the data acquired a network transport nobody had written for. Nothing leaked — the classification changed underneath content already written. **Data put in a channel inherits every transport that channel later gains** (TTC-027). A 600-char excerpt bound does not help: the values sit in the opening, which is exactly what an excerpt keeps.
...
- 📱 **iPhone reports → the iOS Simulator, not Playwright WebKit** (2026-09-12). Playwright WebKit is desktop WebKit; it passed a YouTube Short that sat black on Leland's iPhone. This Mac has Xcode's Simulator (real Mobile Safari): `xcrun simctl boot <udid>` → `xcrun simctl openurl booted <url>` → `xcrun simctl io booted screenshot`. Load private pages through a local `srcdoc` harness (the Simulator cannot be signed in by script), keep a known-good control beside the test, and measure pixels over time.
- 🎬 **A `file://` harness cannot judge YouTube playback** (2026-09-21). The embed refuses to load without an HTTP referrer and shows *"Video player configuration error — Error 153"* for EVERY video, working ones included. Serve the harness (`python3 -m http.server <port>`) and load `http://localhost:<port>/...`; students always send a referrer from `lelandlong.github.io`. `file://` is still fine for DOM, thumbnails, chapter buttons and embed `src` params. Found when a new Short showed Error 153 and the known-good control showed the same.
...
## 12a. 🔴 A procedure lives in exactly ONE file — re-read the WHOLE file when the SYSTEM changes
- **One file owns each procedure; every other file points at it.** If you are editing the same instruction twice, **the second copy is the defect, not the edit**. `CLAUDE.md` should defer to the skill, not restate it.
- **The trigger for a re-read is a change to the SYSTEM, not to the file.** Re-reading only the paragraph you touched is not enough — this seat re-read the sections it edited three times on 2026-08-27 and both survivors sat in sections it had no reason to open.
- ⭐ **The residual is in the file you did NOT open, and it is usually the one read FIRST.** Fixing the skill and leaving `CLAUDE.md` is the worst ordering available: the stale copy is what a session meets first, the fresh copy is what nobody reaches. That happened here (PR #36 fixed the skill; PR #37 caught `CLAUDE.md` an hour later).
- **A stale POSTING rule fails loudly** — the tool is there or it is not. **A stale READING rule fails SILENTLY and SELECTIVELY**: it points at a source that still exists and still has content, so the session reads *something*, concludes it is done, and never learns what it missed. At a seat with no local runtime it degrades to reading nothing, with no error. **Change how a thing is written and you have almost certainly changed how it is read — check both halves in one pass.**
- **A document you are actively correcting READS as corrected** — the freshest section makes the whole file feel current.
- **Two zero-knowledge tests, free and needing no outside facts:**
  1. **Internal contradiction.** A file instructing the same action twice is redundant or wrong. Here, "prefer `monitor.sh` for a manual check" contradicted step 1 of its own file.
  2. **Read your own do-not-touch blocks.** They are the least re-read text you own.
- ⚠️ **And a test over a RANGE cannot be validated by its ENDPOINTS.** Both of this seat's 08-27 errors were that shape: a 60-line scan window used to judge whole messages, and a marker-vs-ledger-*maximum* comparison used to claim no mail was skipped (TTC-041 disproved it by probing every interior id).
...
- 🔴 **END THE REPLY WITH A SUMMARY — always** (template v1.3, Leland's standing order via TTC-019, 2026-08-17: *"I always want a damn summary so I can mentally acknowledge I have read everything I NEED TO READ."*). This applies to EVERY substantive reply, not just ship reports.
  ...
  - **Shape:** a handful of bullets at the very end — what changed · what it means · what (if anything) is on him **for THIS repo**. No re-explaining the body. Anything needing his decision or action is the **last thing on the page**, never buried mid-reply.
  - 🛑 **NEVER report what other sessions are waiting on him for** (Leland, 2026-08-19, SCH-144: *"I dont need a session giving me a list of who else is waiting on me and for what. NOISE."*). The crossfeed monitor exists for that; a session repeating it competes with the tool and buries the one thing only this session can tell him. Chef's Kitchen asks only — no "STK still owes…", no cross-board roundup. *"Nothing from this repo needs you"* is fine when the distinction matters.
  - **And `NEEDS-LELAND` in the status file must MEAN it** — set it only when a decision or action on THIS repo genuinely blocks work, and clear it the moment it stops being true. Rule 1 is only safe because the dashboard is trustworthy.
